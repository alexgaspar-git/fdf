...
/*   Updated: 2022/06/16 22:40:55 by algaspar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	dr_line_one(t_coord line, t_bres bres, t_fdf *fdf)
{
	int		i;
	t_sec	k;
	...
	while (i <= bres.cex)
	{
		...
		if (line.x1 < W && line.y1 < H && line.x1 > -1 && line.y1 > -1)
			my_mlx_pixel_put(fdf->data, line.x1, line.y1,
				get_color(k.current, k.start, k.end));
		line.x1 += bres.xincr;
		bres.ex -= bres.dy;
		if (bres.ex < 0)
		{
...
static void	dr_line_two(t_coord line, t_bres bres, t_fdf *fdf)
{
	int		i;
	t_sec	k;
...
void	dr_line(t_coord line, t_fdf *fdf)
{
	...
	bres = init_bres(line);
	if (bres.ex >= bres.ey)
		dr_line_one(line, bres, fdf);
	if (bres.ex < bres.ey)
		dr_line_two(line, bres, fdf);
}
